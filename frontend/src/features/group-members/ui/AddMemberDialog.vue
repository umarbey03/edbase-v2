 ...
 * Yozilgan matn TELEFONGA o'xshasa telefon maydoniga, aks holda ism
 * maydoniga ko'chadi — xodim uni qayta termaydi.
 */
function switchToNewFromSearch(): void {
  const term = search.value.trim()
  if (/^[+\d\s()-]+$/.test(term) && term.length > 0) phone.value = term
 ...
 * ISMNI qo'yamiz — xodim uni bir bosishda topib qo'shadi. Aks holda u
 * formani qayta yuborib, ikkinchi nusxani yaratib qo'yardi va nima
...
const createAndAddMutation = useMutation({
  ...
  onError: (error: Error) => {
    if (error instanceof CreatedButNotAddedError) {
      ...
      search.value = error.user.fullName ?? ''
      mode.value = 'existing'
      return
    }
    errorMessage.value = toUserMessage(error)
