...
const props = defineProps<{
  /** Xabar yuborish funksiyasi. `true` qaytsa — matn tozalanadi. */
  send: (body: string) => Promise<boolean>
  canSend: boolean
  isSending: boolean
  /** SPEC 6.2 — 2 soniyalik cheklov qoldig'i. */
...
const canSubmit = computed(
  () => !props.disabled && props.canSend && !props.isSending && draft.value.trim().length > 0,
)
